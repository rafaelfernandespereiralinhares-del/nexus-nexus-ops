...
// Defined OUTSIDE component to fix React ref/warning
      ...
      <div className="grid gap-3 grid-cols-3">
        <Card className="border-0 bg-gradient-to-br from-blue-500/10 to-blue-600/5">
          <CardContent className="p-3">
            ...
            <div className="text-base font-bold text-foreground leading-tight">{fmtShort(meta?.meta_diaria ?? 0)}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 bg-gradient-to-br from-emerald-500/10 to-emerald-600/5">
          <CardContent className="p-3">
            ...
            <div className="text-base font-bold text-foreground leading-tight">{fmtShort(realizadoHoje)}</div>
          </CardContent>
        </Card>
        ...
        <CardHeader className="pb-2 pt-4 px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button onClick={() => setChartType('bar')} className={`p-1.5 rounded-lg transition-all ${chartType === 'bar' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} title="Barras">
                <BarChart2 className="h-3.5 w-3.5" />
              </button>
              <button onClick={() => setChartType('line')} className={`p-1.5 rounded-lg transition-all ${chartType === 'line' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} title="Linha">
                <LineChartIcon className="h-3.5 w-3.5" />
              </button>
              <button onClick={() => setChartType('pie')} className={`p-1.5 rounded-lg transition-all ${chartType === 'pie' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`} title="Pizza">
                <PieChartIcon className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
          {chartType === 'pie' && <p className="text-xs text-muted-foreground">Meta vs. realizado hoje</p>}
        </CardHeader>
