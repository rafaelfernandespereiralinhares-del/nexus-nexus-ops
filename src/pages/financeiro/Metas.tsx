...
import { Plus, Download, Upload, Trophy, Pencil, Trash2, Target, Users } from 'lucide-react';
import { exportToCSV, parseCSV, parseExcel } from '@/lib/csv';
import { MetasVendedoresTab } from '@/features/metas-vendedores/components/MetasVendedoresTab';
import { PremiacaoTab } from '@/features/metas-vendedores/components/PremiacaoTab';
...
export default function Metas() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      empresa_id: profile.empresa_id, loja_id: form.loja_id, mes: form.mes,
      meta_mensal: parseFloat(form.meta_mensal) || 0, meta_diaria: parseFloat(form.meta_diaria) || 0, meta_lucro: parseFloat(form.meta_lucro) || 0,
    };
    let error;
    if (editId) { ({ error } = await supabase.from('metas').update(payload).eq('id', editId)); }
    else { ({ error } = await supabase.from('metas').insert(payload)); }
    if (error) { toast({ title: 'Erro', description: error.message, variant: 'destructive' }); }
    else { toast({ title: editId ? 'Meta atualizada!' : 'Meta cadastrada!' }); setDialogOpen(false); resetForm(); fetchMetas(); }
  };

  const handleEdit = (m: any) => {
    setEditId(m.id);
    setForm({ loja_id: m.loja_id, mes: m.mes, meta_mensal: String(m.meta_mensal), meta_diaria: String(m.meta_diaria), meta_lucro: String(m.meta_lucro) });
    setDialogOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold">Metas & Premiação</h1>
          <p className="text-sm text-muted-foreground">Metas por loja, vendedor e configuração de premiação</p>
        </div>
      </div>

      <Tabs defaultValue="mensal" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="mensal" className="gap-2"><Target className="h-4 w-4" /> Metas Loja</TabsTrigger>
          <TabsTrigger value="vendedores" className="gap-2"><Users className="h-4 w-4" /> Vendedores</TabsTrigger>
          <TabsTrigger value="premiacao" className="gap-2"><Trophy className="h-4 w-4" /> Premiação</TabsTrigger>
        </TabsList>

        {/* Aba Metas Mensal (existente) */}
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(m)} title="Editar"><Pencil className="h-4 w-4" /></Button>
                        {primaryRole === 'ADMIN' && (
